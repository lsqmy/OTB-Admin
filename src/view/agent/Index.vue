<template>
  <div class="container" ref="container">
    <Modal
      ...
      width="70"
      @on-cancel="cancel">
      <Form ref="form" :label-width="80" :model="buyerData" :rules="ruleValidate">
        <FormItem prop="site" label="所属站点">
          <Select v-model.trim="buyerData.siteId">
            <Option v-for="item in siteList" :key="item.siteId" :value="item.siteId">{{ item.siteName }}</Option>
          </Select>
        </FormItem>
        <FormItem prop="isAgent" label="是否代理">
          <RadioGroup v-model="buyerData.isAgent">
            <Radio :label="true">是</Radio>
            <Radio :label="false">否</Radio>
          </RadioGroup>
        </FormItem>
        <FormItem prop="agentUserName" label="代理人名称" v-if="buyerData.isAgent">
          <Input v-model.trim="buyerData.agentUserName"></Input>
        </FormItem>

        <FormItem prop="userName" label="用户名">
          <Input v-model.trim="buyerData.userName"></Input>
        </FormItem>
        <FormItem prop="realName" label="姓名">
          <Input v-model.trim="buyerData.realName"></Input>
        </FormItem>
        <FormItem prop="phone" label="电话">
          <Input v-model.trim="buyerData.phone"></Input>
        </FormItem>
      </Form>
      <div slot="footer">
